...
function setup() {
  ...
  gameOver = createSprite(width/2,height/3,400,20);
...
ground.velocityX =-4-(4 + score/1000);
  }
  if (ground.x < 0){
    ground.x = ground.width/2;
  ...
  if(keyDown("Up")&& trex.y >= 450) {
    trex.velocityY = -12;
    jumpSound.play()
  } if(keyDown("W")&& trex.y >= 450) {
    trex.velocityY = -12;
    jumpSound.play()
  }
  intro.visible = false;
...
function spawnObstacles(){
 if (frameCount % 40 === 0){
   var obstacle = createSprite(windowWidth,height-165,10,40);
   
   obstacle.velocityX =-4-(4 + score/1000);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    cloud = createSprite(windowWidth,height-100,40,10);
